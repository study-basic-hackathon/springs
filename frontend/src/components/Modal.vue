    ...
    <div class="post-wrap">
      <div class="inner">
        ...
        <form action="/" method="post" class="post-form" name="postForm" @submit.prevent="post">
          <input type="hidden" name="shopId" :value="place?.place_id" id="post-shop-id">
          <dl class="form-list">
            <div>
              <dt>待ち時間</dt>
              <dd><input type="number" min="0" step="5" name="waitTime" class="post-form-time" required><span>分</span></dd>
            </div>
            <div>
              <dt>コメント</dt>
...
async function post() {
  ...
  await fetch(`${API_BASE}/queue`, {
    ...
    body: JSON.stringify({ placeId: postShopId.value, waitTime: postWaitTime.value, comment: postComment.value }) // 送信するデータ
  })
